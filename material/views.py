...
@wms_permission_required('can_wms_stock_list')
...
@wms_permission_required('can_wms_incoming_history')
def incoming_history(request):
    ...
    context = {
        ...
        'vendors': vendors,
        'parts': parts,
        'erp_failed_count': erp_failed_count,
...
@wms_permission_required('can_wms_transaction_history')
...
@wms_permission_required('can_wms_outbound')
...
@wms_permission_required('can_wms_stock_transfer')
...
@wms_permission_required('can_wms_transfer_history')
...
@wms_permission_required('can_wms_stock_return')
...
@wms_permission_required('can_wms_bom_list')
...
@login_required
@wms_permission_required('can_wms_bom_upload')
...
@wms_permission_required('can_wms_storage_layout')
...
@wms_permission_required('can_wms_storage_expiry')
...
def raw_material_incoming(request):
    ...
    if ImportInspection:
        ...
        for insp in approved_inspections:
            ...
            pallet_labels = RawMaterialLabel.objects.filter(
                incoming_transaction=trx, label_type='PALLET'
            ).exclude(status='CANCELLED')
            insp.pallet_count = pallet_labels.count()
            insp.pallet_label_ids = ','.join(str(l.id) for l in pallet_labels)
            # 품목설정 데이터 (모달 기본값용)
            try:
...
def api_raw_material_auto_stock(request):
    ...
    if request.method == 'GET':
        ...
        for s in settings_qs:
            ...
            if total == 0:
                continue
            monthly_avg = total / 3
            calc_base = min(monthly_avg, 1000)
            results.append({
                'part_id': s.part_id,
                'part_no': s.part.part_no,
                'part_name': s.part.part_name,
                'monthly_avg': round(monthly_avg),
                'suggested_safety': round(calc_base * 0.4 / 25) * 25,
                'suggested_warning': round(calc_base * 0.3 / 25) * 25,
    ...
    for pid in part_ids:
        ...
        if total == 0:
            continue
        calc_base = min(total / 3, 1000)
        RawMaterialSetting.objects.filter(part_id=pid).update(
            safety_stock=round(calc_base * 0.4 / 25) * 25,
            warning_stock=round(calc_base * 0.3 / 25) * 25,
        )
        updated += 1
...
@login_required
@wms_permission_required('can_wms_erp_sync')
...
@wms_permission_required('can_wms_outgoing_history')
...
반드시 존댓말(~습니다/~입니다 체)로 작성하고, 불필요한 인사말 없이 바로 내용만 작성하며, 각 섹션 제목은 【】로 구분해주세요.
{caution}
━━━━━━━━━━━━━━━━━━
...
# =============================================================================
# 재료 이동 요청 (Material Transfer Request)
# =============================================================================
...
@login_required
@wms_permission_required('can_wms_transfer_request')
def transfer_request_list(request):
    ...
    return render(request, 'material/transfer_request_list.html', {
...
def transfer_request_create(request):
    ...
    if request.method == 'POST':
        part_nos = request.POST.getlist('part_no')
        qtys = request.POST.getlist('requested_qty')
        remarks = request.POST.getlist('item_remark')
        overall_remark = request.POST.get('remark', '').strip()

        # 유효한 라인만 파싱
        lines_data = []
        for pno, qty_str, rem in zip(part_nos, qtys, remarks):
            pno = pno.strip()
            if not pno:
                continue
            part = Part.objects.filter(part_no=pno).first()
            if not part:
                messages.error(request, f'품번을 찾을 수 없습니다: {pno}')
                return redirect('material:transfer_request_create')
            try:
                qty = int(qty_str)
                if qty <= 0:
                    raise ValueError
            except (ValueError, TypeError):
                messages.error(request, f'수량이 올바르지 않습니다: {pno}')
                return redirect('material:transfer_request_create')
            lines_data.append((part, qty, rem.strip()))

        if not lines_data:
            messages.error(request, '품목을 1개 이상 추가해주세요.')
            return redirect('material:transfer_request_create')

        with transaction.atomic():
            req = MaterialTransferRequest.objects.create(
                request_no=MaterialTransferRequest.generate_request_no(),
                remark=overall_remark,
                requested_by=request.user,
            )
            for part, qty, rem in lines_data:
                MaterialTransferRequestLine.objects.create(
                    request=req, part=part, requested_qty=qty, remark=rem
                )

        messages.success(request, f'이동 요청이 등록되었습니다. ({req.request_no}, {len(lines_data)}개 품목)')
        return redirect('material:transfer_request_list')

    return render(request, 'material/transfer_request_create.html')
...
def transfer_request_approve(request, pk):
    ...
    # POST: 승인 처리
    wh_from_id = request.POST.get('warehouse_from')
    wh_to_id = request.POST.get('warehouse_to')
    try:
        from_wh = Warehouse.objects.get(pk=wh_from_id)
        to_wh = Warehouse.objects.get(pk=wh_to_id)
    except Warehouse.DoesNotExist:
    ...
    try:
        ...
        logger.exception('transfer_request_approve error')
...
def api_transfer_request_stock(request):
    ...
    part_no = request.GET.get('part_no', '').strip()
    if not part_no:
        return JsonResponse({'ok': False, 'error': '품번을 입력하세요.'})

    part = Part.objects.filter(part_no=part_no).first()
    if not part:
        suggestions = list(
            Part.objects.filter(part_no__icontains=part_no).values('part_no', 'part_name')[:10]
        )
        return JsonResponse({'ok': False, 'error': '품번을 찾을 수 없습니다.', 'suggestions': suggestions})

    target_codes = ['3000', '2000', '3200']
    stocks = []
    for code in target_codes:
        wh = Warehouse.objects.filter(code=code, is_active=True).first()
        if not wh:
            continue
        qty = MaterialStock.objects.filter(warehouse=wh, part=part).aggregate(total=Sum('quantity'))['total'] or 0
        stocks.append({'warehouse_code': wh.code, 'warehouse_name': wh.name, 'qty': int(qty)})

    return JsonResponse({'ok': True, 'part_no': part.part_no, 'part_name': part.part_name, 'stocks': stocks})


@login_required
@wms_permission_required('can_wms_transfer_request')
def api_transfer_request_lots(request):
    """창고 + 품번 → LOT별 재고 반환 (승인 화면용)"""
    from django.http import JsonResponse
    part_no = request.GET.get('part_no', '').strip()
    ...
    part = Part.objects.filter(part_no=part_no).first()
    if not part:
        return JsonResponse({'ok': False, 'error': '품번 없음'})
    try:
