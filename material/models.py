...
class MaterialTransferRequest(models.Model):
    ...
    def __str__(self):
        return f"[{self.get_status_display()}] {self.request_no}"
...
class MaterialTransferRequestLine(models.Model):
    request = models.ForeignKey(
        MaterialTransferRequest, on_delete=models.CASCADE,
        related_name='lines', verbose_name="이동요청"
    )
    part = models.ForeignKey(Part, on_delete=models.CASCADE, verbose_name="품목")
    ...
    # 승인 시 채워짐
    lot_no = models.DateField("승인 LOT", null=True, blank=True)
    ...
    transfer_transaction = models.ForeignKey(
        ...
        related_name='origin_request_lines', verbose_name="이동 트랜잭션"
    )
...
class PurchaseOrderRequest(models.Model):
    ...
    # 품목 정보 (신청 시점 스냅샷 — 이후 Part 삭제되어도 기록 보존)
    part = models.ForeignKey(Part, on_delete=models.SET_NULL, null=True, related_name='purchase_requests', verbose_name="품목")
    ...
    # 신청 내용
    request_qty = models.DecimalField("발주신청 수량", max_digits=15, decimal_places=2)
    reason = models.TextField("신청사유", blank=True)

    # 신청 정보
    requested_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True,
        related_name='purchase_order_requests', verbose_name="신청자"
    )
    requested_at = models.DateTimeField("신청일시", auto_now_add=True)
    ...
    approved_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='approved_purchase_requests', verbose_name="승인자"
    )
    ...
    def __str__(self):
        return f"[{self.get_status_display()}] {self.request_no} {self.part_no}"
