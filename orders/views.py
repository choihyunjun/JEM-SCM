...
def inventory_list(request):
    ...
    if user.is_superuser or not hasattr(user, 'vendor'):
        ...
        standard_end = today + timedelta(days=31); end_date = max_due if max_due and max_due > standard_end else standard_end
    else: end_date = today + timedelta(days=14)
    date_range = [today + timedelta(days=i) for i in range((end_date - today).days + 1)]
    show_all = request.GET.get('show_all') == 'true'; selected_v = request.GET.get('vendor_id')
    if user.is_superuser or not hasattr(user, 'vendor'):
        inventory_items = Inventory.objects.select_related('part', 'part__vendor').all(); vendor_list = Vendor.objects.all().order_by('name'); vendor_name = "전체 관리자"
        if selected_v: inventory_items = inventory_items.filter(part__vendor_id=selected_v)
    elif hasattr(user, 'vendor'):
        inventory_items = Inventory.objects.select_related('part', 'part__vendor').filter(part__vendor=user.vendor); vendor_list = []; vendor_name = user.vendor.name
    ...
    for item in inventory_items:
        daily_status = []; ref = item.last_inventory_date or timezone.datetime(2000, 1, 1).date(); stock = item.base_stock
        ...
        for dt in date_range:
            ...
            stock = stock - dq + iq
            daily_status.append({'date': dt, 'demand_qty': dq, 'in_qty': iq, 'stock': stock, 'is_danger': stock < 0})
    ...
    return render(request, 'inventory_list.html', {'date_range': date_range, 'inventory_data': inventory_data, 'vendor_list': vendor_list, 'active_menu': 'inventory', 'show_all': show_all, 'selected_vendor_id': selected_v, 'user_name': user.username, 'vendor_name': vendor_name})
...
def inventory_export(request):
    user = request.user; today = timezone.now().date()
    max_due = Demand.objects.aggregate(Max('due_date'))['due_date__max']
    end_date = max_due if max_due and max_due > (today + timedelta(days=31)) else (today + timedelta(days=31))
    ...
    for item in items:
        ...
        stock = stock - (Demand.objects.filter(part=item.part, due_date__gt=ref, due_date__lt=today).aggregate(Sum('quantity'))['quantity__sum'] or 0) + (Incoming.objects.filter(part=item.part, in_date__gt=ref, in_date__lt=today).aggregate(Sum('quantity'))['quantity__sum'] or 0)
...
def demand_upload_action(request):
    ...
    if request.FILES.get('demand_file'):
        try:
            wb = openpyxl.load_workbook(request.FILES['demand_file']); ws = wb.active; c_count = 0
            with transaction.atomic():
                for row in ws.iter_rows(min_row=2, values_only=True):
                    part = Part.objects.filter(part_no=row[1], vendor__name=row[0]).first()
                    if part: Demand.objects.update_or_create(part=part, due_date=row[3], defaults={'quantity': row[2]}); c_count += 1
...
def label_list(request):
    ...
    vendor_list = Vendor.objects.filter(id__in=vendor_ids).order_by('name') if (user.is_superuser or (hasattr(user, 'profile') and user.profile.role == 'STAFF')) else []

    recent_orders = DeliveryOrder.objects.all().prefetch_related('items')
    
    if not (user.is_superuser or (hasattr(user, 'profile') and user.profile.role == 'STAFF')):
        my_parts = Part.objects.filter(vendor=user.vendor).values_list('part_no', flat=True)
        recent_orders = recent_orders.filter(items__part_no__in=my_parts).distinct()
    elif selected_v:
        v_parts = Part.objects.filter(vendor_id=selected_v).values_list('part_no', flat=True)
        recent_orders = recent_orders.filter(items__part_no__in=v_parts).distinct()

    if status_filter == 'registered':
        recent_orders = recent_orders.filter(is_received=False)
    elif status_filter == 'received':
        recent_orders = recent_orders.filter(is_received=True)
    
    q = Order.objects.filter(is_closed=False, approved_at__isnull=False)
    if not (user.is_superuser or (hasattr(user, 'profile') and user.profile.role == 'STAFF')):
        q = q.filter(vendor=user.vendor)
    elif selected_v:
    ...
    return render(request, 'label_list.html', {
        ...
        'recent_orders': recent_orders.order_by('-created_at')[:15], 
...
def delete_delivery_order(request, order_id):
    ...
    if order.is_received:
        messages.error(request, "이미 입고가 완료된 납품서는 삭제할 수 없습니다.")
        return redirect('label_list')
    with transaction.atomic():
        for item in order.items.all():
            LabelPrintLog.objects.filter(part_no=item.part_no, printed_qty=item.total_qty, printed_at__date=order.created_at.date()).delete()
        order.delete()
...
def delivery_note_print(request, order_id):
    ...
    return render(request, 'print_delivery_note.html', {
        'order': do, 'items': items, 'total_qty': items.aggregate(Sum('total_qty'))['total_qty__sum'] or 0,
        'total_box': items.aggregate(Sum('box_count'))['box_count__sum'] or 0, 'print_date': timezone.now().date(), 'vendor': vendor,
    })
...
def receive_delivery_order_scan(request):
    ...
    if not do:
        messages.error(request, f"유효하지 않은 납품서 번호입니다: {qr_code}")
        return redirect('incoming_list')
    if do.is_received:
        messages.warning(request, f"이미 입고 처리된 납품서입니다: {qr_code}")
        return redirect('incoming_list')
    with transaction.atomic():
...
def incoming_list(request):
    ...
    if not user.is_superuser and user.profile.role != 'STAFF': 
        incomings = incomings.filter(part__vendor=user.vendor)
    elif selected_v:
    ...
    return render(request, 'incoming_list.html', {
        'incomings': incomings, 'active_menu': 'incoming', 
        'start_date': sd, 'end_date': ed, 'q': q,
        'vendor_list': vendor_list, 'selected_vendor_id': selected_v
    })
...
def incoming_export(request):
    ...
    if not request.user.is_superuser and hasattr(request.user, 'vendor'):
        incomings = incomings.filter(part__vendor=request.user.vendor)
    wb = openpyxl.Workbook(); ws = wb.active; ws.append(['입고일자', '협력사', '품번', '품명', '입고수량', '처리일시'])
    for i in incomings:
        ws.append([i.in_date, i.part.vendor.name, i.part.part_no, i.part.part_name, i.quantity, i.created_at.strftime("%Y-%m-%d %H:%M")])
    response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
def scm_admin_main(request):
    ...
    summary = {
        'total_vendors': Vendor.objects.count(), 'total_parts': Part.objects.count(),
    ...
    return render(request, 'scm_admin_main.html', {
        'summary': summary, 'recent_incomings': recent_incomings, 'active_menu': 'admin_main', 'user_name': request.user.username, 'vendor_name': "시스템 관리자"
    })